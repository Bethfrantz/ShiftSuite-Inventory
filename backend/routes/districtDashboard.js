...
router.get("/:districtId", async (req, res, next) => {
  try {
    ...
    const kpiComparison = {};
    const firstKpisForComparison = storeComparisons[0]?.kpis || {};

    for (const key of Object.keys(firstKpisForComparison)) {
      kpiComparison[key] = {
        label: firstKpisForComparison[key]?.label || key,
    ...
    const anomalyPressureScore = anomalyRanking?.[0]?.value ?? 0;
    const scoreSlope = districtScoreForecastSlope["30d"] === "down" ? 1 : 0;
    const weakKpiCountScore = Object.values(kpiComparison).filter(
    ...
    // DISTRICT QUARTERLY EXECUTIVE REPORT
    // -----------------------------
    const quarterlyExecutiveReportSummary = {
      districtId,
      generatedAt: new Date(),
      period: "Last 90 Days",
    ...
    // -----------------------------
    const anomalyClusterScore = detectClusters(districtAnomalyHeatmap);
